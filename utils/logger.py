...
class EvalMonitor(object):
    def __init__(self, cfg):
        ...
        for measure in cfg['eval episode performance']:
            self.episode_performance[measure] = []

    def collect_step_performance(self, info):
        pass

    def collect_episode_performance(self, info):
        for measure in self.cfg['eval episode performance']:
            self.episode_performance[measure].append(info[measure])

    def dump_end_eval(self, logger, step):
        for measure in self.cfg['eval episode performance']:
            logger.log_histogram(f"eval/{measure}", np.array(self.episode_performance[measure]), step)
            self.episode_performance[measure] = []
